...
/**
 * @author com.mrathena on 2018/12/24 15:00
 */
public final class TextFileKit {

	public static void main(String[] args) {
		TextFileKit.fileHandleInBatch(new File("C:\\Users\\com.mrathena\\Desktop/redis删除.txt"), 1, list -> {
			try {
				...
				TextFileKit.collectionToFile(result, string -> string, new File("C:\\Users\\com.mrathena\\Desktop\\result"), true);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
		...
		try {
			if (!file.exists()) {
				if (!file.createNewFile()) {
					throw new ServiceException("创建文件失败,可能是没有权限");
				}
			}
