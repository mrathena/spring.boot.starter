...
public final class DateTimeKit {
	...
	public static void main(String[] args) {
		...
		System.out.println(DateTimeKit.isLocalDateTimeDurationOverlap(start, stop, anotherStart, anotherStop));
		...
		System.out.println(DateTimeKit.isLocalDateDurationOverlap(start2, stop2, anotherStart2, anotherStop2));
	}
	...
	public static boolean isLocalDateTimeDurationOverlap(LocalDateTime start, LocalDateTime stop, LocalDateTime anotherStart, LocalDateTime anotherStop) {
		if (start == null || stop == null || anotherStart == null || anotherStop == null) {
			throw new ServiceException(ServiceErrorCodeEnum.EXCEPTION, "四个LocalDateTime必须都不为null");
		}
		if (start.isAfter(stop) || anotherStart.isAfter(anotherStop)) {
			throw new ServiceException(ServiceErrorCodeEnum.EXCEPTION, "两组结束时间必须晚于同组开始时间");
		}
	...
	public static boolean isDataDurationOverlap(Date start, Date stop, Date anotherStart, Date anotherStop) {
		if (start == null || stop == null || anotherStart == null || anotherStop == null) {
			throw new ServiceException(ServiceErrorCodeEnum.EXCEPTION, "四个LocalDateTime必须都不为null");
		}
	...
	public static Set<String> getYearlyStrListByRange(LocalDate begin, LocalDate end) {
		if (begin.isAfter(end)) {
			throw new ServiceException("begin must before at end");
		}
		int capacity = end.getYear() - begin.getYear();
		Set<String> set = new LinkedHashSet<>(capacity);
		while (!begin.isAfter(end)) {
			set.add(begin.format(FORMATTER_YEAR));
			begin = begin.plusYears(1);
		}
		return set;
	}

	public static Set<String> getMonthlyStrListByRange(LocalDate begin, LocalDate end) {
		if (begin.isAfter(end)) {
			throw new ServiceException("begin must before at end");
		}
		long capacity = ChronoUnit.MONTHS.between(begin, end);
		Set<String> set = new LinkedHashSet<>((int) capacity + 1);
		while (!begin.isAfter(end)) {
			set.add(begin.format(FORMATTER_COMPACT_YEAR_MONTH));
			begin = begin.plusMonths(1);
		}
		return set;
	}

	public static Set<String> getDailyStrListByRange(LocalDate begin, LocalDate end) {
		if (begin.isAfter(end)) {
			throw new ServiceException("begin must before at end");
		}
		long capacity = ChronoUnit.DAYS.between(begin, end);
		Set<String> set = new LinkedHashSet<>((int) capacity + 1);
		while (!begin.isAfter(end)) {
			set.add(begin.format(FORMATTER_COMPACT_DATE));
	...
	/**
	 * 输入 2020,2022 输出 2020,2021,2022
	 */
	public static Set<String> getYearlyStrListByRange(String begin, String end) {
		return getYearlyStrListByRange(LocalDate.parse(begin + "0101", FORMATTER_COMPACT_DATE), LocalDate.parse(end + "0101", FORMATTER_COMPACT_DATE));
	}

	/**
	 * 输入 202001,202003 输出 202001,202002,202003
	 */
	public static Set<String> getMonthlyStrListByRange(String begin, String end) {
		return getMonthlyStrListByRange(LocalDate.parse(begin + "01", FORMATTER_COMPACT_DATE), LocalDate.parse(end + "01", FORMATTER_COMPACT_DATE));
	}

	/**
	 * 输入 20200101,20200103 输出 20200101,20200102,20200103
	 */
	public static Set<String> getDailyStrListByRange(String begin, String end) {
		return getDailyStrListByRange(LocalDate.parse(begin, FORMATTER_COMPACT_DATE), LocalDate.parse(end, FORMATTER_COMPACT_DATE));
	}
